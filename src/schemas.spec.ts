import { createCustomerDto } from './customers/schemas/create-customer.dto';
import { updateCustomerDto } from './customers/schemas/update-customer.dto';
import { createProductDto } from './products/schemas/create-product.dto';
import { updateProductDto } from './products/schemas/update-product.dto';
import { createPurchaseOrderDto } from './purchase-orders/schemas/create-purchase-order.dto';
import { createSaleOrderDto } from './sale-orders/schemas/create-sale-order.dto';
import { updateSaleOrderDto } from './sale-orders/schemas/update-sale-order.dto';
import { createSupplierDto } from './suppliers/schemas/create-supplier.dto';
import { updateSupplierDto } from './suppliers/schemas/update-supplier.dto';
import { createUserDto } from './users/schemas/create-user.dto';
import { updateUserDto } from './users/schemas/update-user.dto';
...
describe('DTO schemas', () => {
  ...
  it('allows partial update payloads while omitting immutable fields', () => {
    expect(updateCustomerDto.parse({ document: 'ignored', name: 'Novo' })).toEqual({
      name: 'Novo',
    });
    expect(updateSupplierDto.parse({ cnpj: 'ignored', city: 'Maceió' })).toEqual({
      city: 'Maceió',
    });
    expect(updateUserDto.parse({ cpf: 'ignored', password: 'OutraSenha2@' })).toEqual({
